...
const REQUIRED_SCHEMA = {
	users: ["id", "email", "enabled"],
	system_settings: ["key", "value", "is_secret"],
	products: ["id", "product_type", "status", "tag_names"],
	product_sellable_items: ["id", "product_id"],
	shop_orders: ["id", "order_number", "status"],
	customer_entitlements: ["id", "user_id", "status"],
	automation_jobs: ["id", "entitlement_id", "status"],
	notification_deliveries: ["id", "channel", "status"],
	outbox_events: ["id", "event_type", "status"],
	supplier_accounts: ["id", "provider", "enabled"],
...
export async function backupNodeData(
	...
	try {
		...
		const objectChecksums = await validateObjectStore(
			join(staging, basename(layout.objects)),
		);
		const files = await checksums(staging, new Set(), objectChecksums);
		await writeFile(
...
function validateDatabase(database: Database, migrations: NodeMigration[]) {
	...
	const unknown = [...applied.keys()].filter((name) => !knownNames.has(name));
	if (unknown.length > 0)
		throw new Error(
			`Database contains unknown migrations: ${unknown.join(", ")}`,
		);
...
function seedNodeMigrations(database: Database, migrations: NodeMigration[]) {
	...
	const knownNames = new Set(migrations.map(({ name }) => name));
	const unknown = [...applied].filter((name) => !knownNames.has(name));
	if (unknown.length > 0)
		throw new Error(
			`D1 export contains unknown migrations: ${unknown.join(", ")}`,
		);
...
function validateGmshopSchema(database: Database) {
	for (const [table, requiredColumns] of Object.entries(REQUIRED_SCHEMA)) {
		...
		if (missing.length > 0)
			throw new Error(
				`Database table ${table} is missing required columns: ${missing.join(", ")}`,
			);
	}
}
...
async function importR2Directory(
	...
	for (const filename of await listFiles(source)) {
		...
		await objects.put(
			key,
			Readable.toWeb(createReadStream(filename)) as ReadableStream,
			manifest.get(key),
		);
...
async function validateObjectStore(root: string) {
	const validated = new Map<string, Omit<BackupFile, "name">>();
	if (!(await exists(root))) return validated;
	const metadataRoot = join(root, "metadata");
	if (!(await exists(metadataRoot))) return validated;
	for (const filename of await listFiles(metadataRoot)) {
		...
		if (checksum.bytes !== metadata.size || checksum.sha256 !== metadata.etag)
			throw new Error(`Object checksum mismatch: ${metadata.key}`);
		validated.set(resolve(dataPath), checksum);
	}
	return validated;
}
...
async function checksums(
	...
	known: ReadonlyMap<string, Omit<BackupFile, "name">> = new Map(),
): Promise<BackupFile[]> {
	...
	for (const filename of await listFiles(root)) {
		...
		files.push({
			name,
			...(known.get(resolve(filename)) ?? (await checksumFile(filename))),
		});
	}
...
async function assertOutsideDataDirectory(
	...
	if (nested) throw new Error("Backup output must be outside GMSHOP_DATA_DIR");
}
...
function requiredDataDirectory(
	...
	const value = options["data-dir"] ?? environment.GMSHOP_DATA_DIR;
	if (typeof value !== "string" || !value)
		throw new Error("Set GMSHOP_DATA_DIR or pass --data-dir explicitly");
...
GMSHOP_DATA_DIR supplies --data-dir when the option is omitted.`);
}
