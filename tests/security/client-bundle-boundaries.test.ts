...
describe("client bundle boundaries", () => {
	it("keeps heavy client dependencies with their semantic owners", () => {
		...
		expect(runtimeImportOwners("react-markdown")).toEqual([
			"src/components/pro/viewer/markdown/index.tsx",
		]);
	...
	it("keeps removed Scalar browser tooling out of the storefront", () => {
		expect(read("package.json")).not.toMatch(/@scalar\//);
		for (const file of sourceFiles) {
			const source = readFileSync(file, "utf8");
			expect(source, projectPath(file)).not.toMatch(
				/@scalar\/api-reference|scalar\.com|cdn\.jsdmirror\.com\/npm\/@scalar/,
			);
		}
	});
